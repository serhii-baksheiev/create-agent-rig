...
clean `--layer workflow` install, at exactly 96 file entries down to 62
...
goes from 96 manifest entries to 62"; the figure moved by one from an
...
`failure-diagnostician` joined it, and by two more again (94→96, 60→62)
when RP-195 slice 3 added the `diagnose` skill's two shipped copies to the
process layer).

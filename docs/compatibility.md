...
`contracts/session-messaging/`) — see the consumer graph in the appendix for
why that framework was dead code, not a working compatibility check.
...
Each row names one `PreToolUse` guard shipped in
`templates/agent-os/universal/.claude/hooks/`, wired identically in
`.claude/settings.json` (Claude Code) and `.codex/hooks.json` (Codex, POSIX
`command` plus a `commandWindows` PowerShell equivalent). Every guard here has
at least one allowed and one denied execution fixture, invoked through the
real wiring string of both harnesses, in
`test/template/guard-acceptance.test.ts`. That file also covers `guard-bash`
and `guard-rulebook`'s own richer behaviour by pointer rather than by
duplication — see the file-specific suites below.
...
| ---------------------------------------- | ----------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PreToolUse` (the six guards above)      | SUPPORTED   | SUPPORTED                  | `guard-acceptance.test.ts`                                                                                                                                      |
...
installed hook or script ever imported it. The benchmark spawned the six

...
own prefix bytes exactly (including anything the user appended AFTER the end
marker, carried through unchanged — round 2), never unlinks the file;
anything else (the markers missing or malformed, the body edited since
install, or the bytes not valid UTF-8) is `preserved`. Because that
`'remove'` verdict leaves the file itself in place, the CLAUDE.md/AGENTS.md
pair `note` (round 4, blocker 2, exemplified below) gets a THIRD case beyond
"removed" and "not a clean removal": removing the nested shim in the same
run a region-tracked AGENTS.md is stripped leaves no readable rulebook
...
| `init`                           | `[--dry-run] [--layer workflow] [--no-color] [-h/--help]` (`--force` is refused and names `upgrade`) | exit 1                                                                                      | no                                                                                                               | Installs or refreshes the process layer, and the workflow layer with `--layer workflow`; a plain re-run keeps a workflow layer a previous run already recorded rather than dropping it. Idempotent: a rig already matching the release reports every file `unchanged` (`## Ownership verdicts`). A pre-existing `AGENTS.md` no longer refuses (RP-256 slice 2): its bytes become the prefix of a bounded, marked managed region, with this release's rendered rulebook appended after them — the one refusal that remains is markers `init` cannot safely merge with, on a FOREIGN AGENTS.md this rig's own manifest does not already name (a region already there, or a malformed fragment of one). Re-running `init` on an AGENTS.md the manifest already tracks is judged by which bucket names it, not by that foreign-marker check: an already region-tracked one is always left alone (idempotent, and never refused even when the user has since edited inside it); an already whole-file rig-owned one (the pre-slice-2 shape) keeps its exact pre-slice-2 behaviour — unedited is left alone, edited is refused outright (round 2, code-reviewer B2/B3). AGENTS.md bytes that are not valid UTF-8 are refused outright — non-zero exit, nothing written — rather than silently corrupted by a lossy re-encode (round 2, code-reviewer B4/security-scanner B2). A pre-existing root `CLAUDE.md` also no longer refuses — it is left untouched and the shim installs nested at `.claude/CLAUDE.md` instead (`## Harness delivery and provider ownership`; `docs/decisions/agents-md-canonical.md`, "AGENTS.md coexistence — the managed region (RP-256 slice 2)"). `--help`/`-h` prints usage to stdout and exits 0, before anything else runs — never a `parseArgs` failure, and no side effects. With `--json` present, `--help`/`-h` is not honoured (RP-239 round 3): `init` accepts no `--json` flag of its own, so the invocation is refused exactly as it was before — `--json` itself as an unrecognised option — never executed. |
| `upgrade`                        | `[--dry-run] [--yes] [--no-color] [-h/--help]`                                                       | exit 1                                                                                      | no                                                                                                               | Replaces files this rig installed and the user did not touch; reports the rest, never merges. Idempotent: a repeat run against an already-upgraded rig reports `unchanged` throughout. A region-tracked AGENTS.md (RP-256 slice 2) is re-verified immediately before it is written, not trusted from the plan — the new body is rendered first, then the file is re-read, re-decoded strictly (non-UTF-8 bytes refuse the write, never corrupt it) and re-located, and only then written atomically (a temp file plus a rename, so a hard-linked AGENTS.md is replaced rather than written through, and its mode is preserved); an edit inside the managed region, its markers going missing, or its bytes turning non-UTF-8 between the plan and `--yes` refuses that one write and prints it rather than silently overwriting or silently losing it, and content the user appended AFTER the end marker carries through the refresh unchanged; see `docs/decisions/agents-md-canonical.md`, "AGENTS.md coexistence — the managed region (RP-256 slice 2)". `--help`/`-h` prints usage to stdout and exits 0, touching nothing (not even the manifest). With `--json` present, `--help`/`-h` is not honoured (RP-239 round 3), for the same reason as `init` above: `upgrade` accepts no `--json` flag either, so the invocation is refused exactly as it was before — an unknown option — never executed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

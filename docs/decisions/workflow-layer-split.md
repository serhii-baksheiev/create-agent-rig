...
clean `--layer workflow` install, at exactly 92 file entries down to 58
...
goes from 92 manifest entries to 58"; the figure moved by one from an
...
workflow-only — and by two more again (90→92, 56→58) when
`failure-diagnostician` joined the process layer).

...
clean `--layer workflow` install, at exactly 94 file entries down to 58
...
goes from 94 manifest entries to 58"; the figure moved by one from an
...
skill joined the process layer, and by two more again (92→94, 58 unchanged)
when RP-195 slice 5 added the `plan-slices` skill's two shipped copies to
the workflow layer — workflow-only, same as `loop` and `pr-ship`, so the
core-only figure does not move).

...
clean `--layer workflow` install, at exactly 96 file entries down to 60
...
goes from 96 manifest entries to 60"; the figure moved by one from an
...
`failure-diagnostician` joined it, and by two more (94→96, 60 unchanged)

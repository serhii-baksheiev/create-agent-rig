...
reports itself as EMPTY.** That is this repo's own measurement, not a caution —
`selectNext(8 × elevated, {lastCompletedTier:'elevated'})` returned
`candidates: 0, skipped: 8` and the stop condition read `queue-empty` with eight
items open. The markers are not negotiable (understating a tier is a failure
recorded twice here).

**This block is not in that shape, and the count is worth having in front of you:
6 of its 32 items are `[elevated]` — AR3-2, AR3-3, AR3-6, AR3-14, AR3-15,
AR3-16.** So the stall the paragraph above describes does not arise here, and
**the owner's ruling is not to interleave deliberately**: take the block in its
own order and let the spacing rule do its own work. Brief §3's sequencing is a
*dependency* order, not a *take* order. The ruling this still needs — whether a
stop can tell "nothing left" from "everything left is spaced out" — is in the
Operator queue.
...
- AR3-2 [elevated]: `doctor` — the harness audits itself, shipping with the test-neighbour requirement covering `.husky/` and `.claude/hooks/` from day one; exemptions are an explicit file list with reasons. Rides §7a's budget-source correction: every declared budget names where its number comes from
...
- **decide: should a stop tell "nothing left" from "everything left is spaced out"?** The shape that raised this is gone — correcting AR2-4/6/7's markers had made all eight items elevated, and `selectNext` refuses an elevated item when the last completed one was elevated, so the run drained one item and stopped. Simulated against the real module rather than inferred: `selectNext(8×elevated, {lastCompletedTier:'elevated'})` → `ticket: null, candidates: 0, skipped: 8`, and `stopConditionOf({candidates:0})` → `kind: 'queue-empty'`, *"no item survives the filters … do not invent work"*. **The v4 block dissolves it** (6 of 32 elevated), and the owner has ruled **not to interleave deliberately** — so half of this is settled. What is not: a session in that state ends reporting an empty queue with work open, and from the stop line alone the operator cannot tell the two apart. Only one of them means the owner must refill, so the distinction is the whole value of the report
- **decide (was AR2-3, now AR3-6's fifth rule, held out of the block): which document gets the jsdom rule.** ⚠ **The brief now agrees with this finding and no longer names a target** — v4 records it as a defect the brief itself authored, twice (v3 and v4), and carries the content question rather than a path. The item routes it to "`stack/node-ts` web rules", and that section does not exist — `templates/agent-os/stack/node-ts/.claude/rules/node-ts.md` is 57 lines with no `web`, `jsdom`, `DOM` or `browser` in it. The web rules live in `templates/agent-os/universal/.claude/rules/architecture.md`. **The tier half of this question is now closed: AR-12 landed, so both candidate homes are declared elevated and the choice no longer changes the gate.** What remains is where the rule belongs and whether it is wanted at all. The rule's content needs a ruling too: `architecture.md` already puts component-level DOM testing deliberately out of scope, so "jsdom implements the DOM, not layout" reads either as reinforcing that exclusion or as licence to adopt jsdom

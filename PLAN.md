...
> **0.9.0, 10 Sep:** RP-169 Done (#203); RP-172 Done (#205); RP-56 Done (#204); #202, RP-59 and
...
- **npm registry publish** — done through `0.8.0`, the current `latest`. Each release is an owner action, because `npm publish` needs 2FA and is irreversible: an agent prepares the release and stops at that command. The release checklist lives in `CHANGELOG.md`.

...
>
> **Status (0.7.1 published 2 Sep 2026; queue moved to Jira `AR` on 16 Aug 2026).** `0.7.1` is a repo-local hotfix for RP-94 — `pr-ship` named owner-directed work with no queue item as a legitimate path while its BEFORE_PR step could not execute one. It is live on npm and **0.7.1 is `latest`**, published from `gitHead` `52e879b6` with `dist.shasum` `afdfa1e9`, 245 files. It is off the version axis: it neither extends nor closes RP-86 or any `0.8.0+` milestone. Post-publish acceptance ran against the **registry artifact**, not a checkout: both targets scaffolded from the published tarball pass their own `pnpm check`, a rig installed from published 0.7.0 upgrades to 0.7.1 replacing exactly the three files the release changes, and the defect itself is reproduced as fixed — `--owner-directed` is `unrecognised` on 0.7.0 and exits 0 on 0.7.1. Phases 0–7 are all shipped, plus the factory extraction (§7.5, §7.6), the port brief (§7.7) and the upgrade brief (§7.8). **Published on npm** — `0.1.0` through `0.7.1` are live and 0.7.1 is `latest`, published from `gitHead` `52e879b6` with `dist.shasum` `afdfa1e9`; the release before it, 0.7.0, came from `gitHead` `6589db36` with `dist.shasum` `34c94881`. **`0.8.0` is prepared and not yet published** — the RP-86 Policy Foundation cut, prepared on `release/0.8.0` with both manifests at `0.8.0`, a `## 0.8.0` entry in `CHANGELOG.md` and `templates/hash-history.json` regenerated to cover `0.7.1`; it is waiting on the owner, because step 8 is the owner's. Until it is on the registry, `0.7.1` remains `latest`. Step 8 in `CHANGELOG.md` — `npm publish` itself — is the owner's alone, because it needs 2FA and is irreversible, and it stayed so for `0.7.1`. Step 9's checks, smoking the published artifact, have been run for `0.7.1` against both targets and the `upgrade` path, with the evidence in `journal/2026-09.md` — the same checks for `0.8.0` are outstanding until it reaches the registry; `CHANGELOG.md` still marks step 9 an owner step, and reconciling those two is the owner's call rather than this file's. `npx create-agent-rig` resolves from the registry, and the git path still works unchanged. An installed rig is brought forward by `create-agent-rig upgrade` rather than by a procedure in a release note. Releases here ship **untagged** — a standing owner decision, since the owner publishes by hand; `CHANGELOG.md` step 7 records what that costs the released-hash table and AR-35 carries the fix. Detailed field notes and per-brief findings live in `NOTES.md`; this file is the map, `NOTES.md` is the log.
...
- **npm registry publish** — done through `0.7.1`, the current `latest`. `0.8.0` is prepared and waiting on that same owner step. Each release is an owner action, because `npm publish` needs 2FA and is irreversible: an agent prepares the release and stops at that command. The release checklist lives in `CHANGELOG.md`.

...
about — the fingerprint is the only memory dedup has here, and without it the
next run files the same proposal again as a fresh `seen ×1`. The marker is one
fixed form at the head of the line, not free text at the end, so the sweep
AR-48(c) proposes can find carried bullets mechanically and check that the key
is alive and that the ticket's fingerprint still matches the line's. 🔴 **The
`· fingerprint: … · seen ×N` tail is never edited by hand** — a line becomes a
candidate only by matching that whole tail (`TAIL`, `plan-md.mjs:254`), and the
comparison then runs on the fingerprint field inside it (`plan-md.mjs:406`,
`core.mjs:575`). A character changed in the tail's fixed shape, or inside the
fingerprint, is a duplicate on the next filing. (The `seen ×N` digit is the one
part that survives tampering — `TAIL` still matches and the fingerprint still
compares equal, so the next filing increments from the falsified number
instead. Quieter, not safer.)

...
>
> **Status (0.7.0 published 1 Sep 2026; queue moved to Jira `AR` on 16 Aug 2026).** Phases 0–7 are all shipped, plus the factory extraction (§7.5, §7.6), the port brief (§7.7) and the upgrade brief (§7.8). **Published on npm** — `0.1.0` through `0.7.0` are live and 0.7.0 is `latest`, published from `gitHead` `6589db36` with `dist.shasum` `34c94881`. The owner's step 8 in `CHANGELOG.md` is done and step 9 — smoking the published artifact — was run against both targets and the `upgrade` path; neither step is an agent's to take, and step 9's evidence is in `journal/2026-09.md`. `npx create-agent-rig` resolves from the registry, and the git path still works unchanged. An installed rig is brought forward by `create-agent-rig upgrade` rather than by a procedure in a release note. Releases here ship **untagged** — a standing owner decision, since the owner publishes by hand; `CHANGELOG.md` step 7 records what that costs the released-hash table and AR-35 carries the fix. Detailed field notes and per-brief findings live in `NOTES.md`; this file is the map, `NOTES.md` is the log.

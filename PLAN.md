...
🔴 **Read this before taking anything from the block: the queue below is mostly
`[elevated]`, and an all-elevated queue reports itself as EMPTY.** That is this
...
- AR3-8: `file-triage` on the queue seam — `triage`-labelled proposals the loop cannot act on, fingerprint dedup, never `ready`, promotion is human; one implementation above the adapters. Measured: the dead channel produced one item in the loop's whole history; the live one was used twice on day one

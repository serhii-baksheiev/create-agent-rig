...
- 🔴 **Three of this entry's own numbers were wrong, and a later `prose-reviewer`
  round found them — so the correction stays visible rather than being quietly
  overwritten.** As first written this bullet said "four commits", `cost` said
  one CI run, and both said the merge waited on GitHub "not on the work". The
  commit count was four short of nothing but the CI count was a 4× understatement
  of the one field the README calls the cheapest signal that a task fought its
  tests, and the "not on the work" half was false — see **the head commit no gate
  round saw**. An entry written to prove that five of seven blockers were the
  run's own prose closed with three unchecked figures of its own.
...
- **queue hygiene** — one finding, and it is thinner than it first read. AR-65's own
  ...
  **`Done`**, closed 28 seconds after it was filed, so `buildJql` excludes it twice
  ...
  file, in the AR-73/AR-56 entry. Reported, not deleted; this entry first presented it
  as queue rot awaiting the owner, and it is not. This session filed the same junk as
  AR-97 and removed its own.

...
- **reviewed** — two branches, two rounds each, four `prose-reviewer` rounds in all:
  ...
  Rounds 2, 3 and 4 ran under the verdict contract #77 had just merged, so each answer
  was validated with `node .claude/scripts/verdict.mjs check … prose-reviewer` before
  anything was read from it, exit 0 each time. Round 1 predates the contract and
  returned prose.

  🔴 **The last two rows are this entry reporting on its own gate, and the honest part
  is what that costs.** An earlier draft carried a disclaimer instead — that the branch
  faced its gate after the text was written, so nothing about it appeared here — and
  round 2 held on exactly that: four bullets above, the entry already reported round
  1's HOLD, so the disclaimer contradicted its own document. Reporting the rounds is
  the only self-consistent form, and it has a cost that cannot be written away: the
  text you are reading was edited after the last round this branch gets, and **the
  edit is unreviewed**. `pr-ship` step 0 was run on it anyway, and refused:
  `GATE ROUNDS EXHAUSTED — 3 rounds on docs/journal-session-tail, cap is 2:
documented-stall.` The remedy the `loop` skill gives for an exhausted cap is to
  escalate to a human rather than review once more, so that is where this went — see
  the PR. Same shape as `a5351b2` above, disclosed at the same volume.
  ...
  **the refused attempt records a round before it exits.** `recordGateRound` runs ahead
  ...
  performed. `feat/ar-65-verdict-schema` also reads 3 but for a different reason — its
  refused attempt was followed by an authorised round, so there the two happen to
  agree. Two branches at 3 meaning different things is the ambiguity AR-100 was filed
  to remove.

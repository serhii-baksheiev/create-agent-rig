...
- **reviewed** — #78 spent both of the rounds its branch gets:
  ...
  This entry's own branch faces its gate after this text is written, so nothing about
  that round appears here: an entry cannot carry the outcome of a review it has not
  had, and writing it in afterwards would be the edit-after-the-last-gate move
  recorded below. The verdict is on the PR.

  Round 2 ran under the verdict contract #77 had just merged, so its answer was
  validated with `node .claude/scripts/verdict.mjs check … prose-reviewer` before
  anything was read from it, exit 0. Round 1 predates the contract and returned prose.

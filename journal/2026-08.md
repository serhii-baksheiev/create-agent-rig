...
- 🔴 **The finding is that a journal entry failed its own gate, and every blocker was
  ...
  **No ticket covers that class.** AR-99 does not: its scope is an enforcement sentence
  ...
  returned `UNMEASURED` on three of its claims; `prose-reviewer` then returned HOLD on
  four more. Every one was caught only because something was pointed at it.
...
- **reviewed** — two branches, five `prose-reviewer` rounds:
  ...
  | `docs/journal-session-tail` | 1              | HOLD, 4 blockers, 1 advisory   |
  | `docs/journal-session-tail` | 2              | HOLD, 1 blocker, 1 advisory    |
  | `docs/journal-session-tail` | 3 (authorised) | HOLD, 2 blockers, 2 advisories |

  Every round from the second on was validated with
  `node .claude/scripts/verdict.mjs check … prose-reviewer` before anything was read
  from it, exit 0 each time — the contract #77 merged. Only `docs/journal-ar-65`'s
  round 1 predates it, and that answer was prose.

  **Round 3 was authorised by the owner after `pr-ship` step 0 refused it.** The
  branch's two rounds were spent on the entry and on the fix for the entry, and the
  `loop` skill's remedy for an exhausted cap is escalation to a human rather than a
  self-granted pass — so it was escalated on the PR with the options stated. **The cap
  was not raised and the counter was not reset**: `.claude/queue.json` carries no
  `maxGateRounds`, so the next branch gets the same two rounds this one did. Same exit
  AR-65 took, and AR-100 is the filed follow-up for the authorisation living in a PR
  comment written by the audited run.

  ⚠ Note the refusal read `3 rounds … cap is 2` on a branch that had had two:
  **the refused attempt records a round before it exits.** `recordGateRound` runs ahead
  of the verdict in `.claude/scripts/queue/index.mjs`; what the comment above it
  validates first is the cap's _configuration_, not the count. So this branch reads 3
  having been reviewed twice, and a counter is a record of _attempts_, not of reviews
  performed. Here that gap closed by accident: the authorised round brought the reviews
  to three, so the counter and the reviews agree again — which is exactly how
  `feat/ar-65-verdict-schema` also came to read 3. A number that lands on the right
  answer for the wrong reason is the ambiguity AR-100 was filed to remove.

  ⚠ **Every blocker this branch collected — four, then one, then two — was in this
  entry's account of its own gate, never in its record of the work.** Each was found in
  the correction before it, which is the non-convergence the round cap exists to detect,
  and the reviewer said so after the first round: the fix is fewer such sentences, not
  better ones. So this bullet is now mostly what was cut. Deleting cannot introduce the
  class of defect that adding kept introducing (`.claude/rules/invariants.md`, "prefer
  deleting a rule to adding one"), which is the one honest thing to do with text that
  has no round left to face.
...
- **cost** — **5 reviewer subagents**, all `prose-reviewer`: 2 on #78 and 3 on this
  branch, matching the five rows in **reviewed** above. **1 `check-premises` pass**, at
  the second entry point, on this entry's own prose — it returned `UNMEASURED` with
  three blockers, one of which was this field claiming zero. 0 `test-writer` runs, not
  applicable to a docs change. 4 completed full local suite runs, plus 1 abandoned
  mid-flight when `origin/master` was merged in. **CI runs consumed: 5 up to this
  commit** — 1 on `docs/journal-ar-65` (`ab0f801`), 2 on this branch (`7b8afa6`,
  `9edc298`) and 1 `push` run on `master` per merge (`2c13183`, `cda2f01`); all green.
  Every further push here adds one, so the PR is the authority for the final figure and
  this line is not. 0 deploys.

  ⚠ **Most of that is this session's word.** Only two of `cost`'s figures can be
  re-checked from outside — the CI runs, from the Actions API, and the 0 deploys. The
  reviewer subagents, the `check-premises` pass and the local suite runs leave no
  artifact a later reader can reach. The round counts in **reviewed** above are the
  same, for a different reason: they live in `.claude/gate-rounds.json`, which is
  gitignored, as is `.claude/runs/`. This note is not a completeness claim about the
  entry — only about the `cost` and `reviewed` fields it is attached to.

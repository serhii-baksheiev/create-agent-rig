...
- **the change** — the CLI resolved its config from `import.meta.url` and the plan
  ...
  direct library imports. Verified by running it, not only by the suite:
  `cd packages/cli && node ../../.claude/scripts/queue/index.mjs next` printed
  `queue-unreadable` before and prints the ticket after.
...
- **`check-premises` paid for itself twice, and both times by stopping work.** On
  AR-79 it returned `PREMISE FALSE` on seven claims — `verdict.mjs record` does not
  exist, AR-65's record shape has no SHA and refuses a sixth key, the "merge route
  set" the ruling said to reuse was reverted for 19 false positives, and the sweep
  half is impossible against a gitignored journal. Nothing was built. On AR-67 it
  split the item: the plan-path half was reproduced and kept, the test-inventory
  half was contradicted (three tests flip under uid 0, not one) and moved out.
...
- **queue hygiene** — the standing kind, not a stale marker. Six open items were
  ...
  ticket that carried it and six were filed fresh: AR-101 and AR-102 (the substrate
  AR-79 and AR-69 both wait on), AR-103 (raise the round cap to the 3 the owner
  ruled — both conditions AR-69 set for revisiting are now met), AR-104, AR-105,
  AR-106, then AR-107, AR-108, AR-109. The three `operator-queue` items were left
  invisible on purpose: they need acts the loop cannot perform.
...
- **cost** — 8 reviewer subagents, 2 `check-premises` passes, 1 `test-writer` run;
  CI runs are on the PRs.

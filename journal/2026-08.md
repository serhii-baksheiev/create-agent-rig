...
- **reviewed** — two branches, two rounds each, four `prose-reviewer` rounds in all:
  ...
  edit is unreviewed**. `.claude/gate-rounds.json` reads 2 against a cap of 2, so
  `pr-ship` step 0 refuses a third, and the remedy the `loop` skill gives for an
  exhausted cap is to escalate to a human rather than review once more. That is what
  happened — see the PR. Same shape as `a5351b2` above, disclosed at the same volume.

...
- **done** — nothing merged. #77 is open with four commits and every local check
  green; the merge waits on GitHub, not on the work (see **stopped at**).
...
- 🔴 **The round-3 blocker is the one worth carrying forward.** Both readers found
  ...
  snippet of the input INTO the `SyntaxError` — twenty characters echoed whole, or
  a sixteen-character window the author positions by choosing where the parse fails
  — so the escape sequences the sanitiser exists to strip travelled out through the
  message _about_ them. It was also the most reachable of the five sites: the others
  need a block that parses, this one needs a block that does not.
...
- **the cap fired, and was not raised** — `pr-ship` step 0 refused round 3 with
  `GATE ROUNDS EXHAUSTED — 3 rounds, cap is 2`, whose text says in as many words
  that raising the cap for _this_ item is the move it exists to prevent. The round
  ran on an explicit owner authorisation instead, recorded on the PR;
  `.claude/gate-rounds.json` still reads 3 against a cap of 2 and `queue.json` is
  untouched, so the next branch gets the same two rounds this one did.

- **queue hygiene** — none found. AR-65's links were accurate.
...
- **stopped at** — not a stop condition of this loop's own. GitHub is in a
  **Partial System Outage** (incident opened 13:40 UTC, Actions / API Requests /
  ...
  check must be `completed/success` for the head commit. It was green for
  `18a2035`; the two commits after it cannot register a run while Actions are down.
  Merging on anything weaker is the "green wall nobody built" the rule names, so
  the branch is pushed, the PR is open and labelled `human-review`, and the merge
  is outstanding.

- **cost** — 3 gate rounds; 8 reviewer subagents (code-reviewer ×3,
  prose-reviewer ×2, security-scanner ×3); 2 `check-premises` passes; 3
  `test-writer` runs; 1 CI run consumed, green, on `18a2035`; 0 deploys.
...
- **queue hygiene** — `AR-71 | proposal: probe` sits in `triage`: junk from an
  earlier session debugging `proposeTriage`. Reported, not deleted. This session
  filed the same junk as AR-97 and removed its own.

...
- **items** — AR-84, then AR-60, then AR-64. The first two were not selected: the
  ...
  the queue in the ordinary way, and the spacing ration allowed it because the
  two before it were normal.
...
- **done** — AR-84 (#65) replaced two absolute millisecond budgets with a bound
  ...
  `## Journal` out of `PLAN.md` into `journal/YYYY-MM.md`: **102,261 → 38,576
...
- **cost** — 5 reviewer subagent launches (`test-writer` ×2, `code-reviewer`
  ×2, `prose-reviewer` ×1), resumed across 8 further rounds; `check-premises`
  applied 3 times as a skill, counted separately because it is not an agent.
  3 PRs, 24 check runs consumed (4 named checks × 6 pushes), **0 re-runs** — no
  task fought its tests. 0 deploys. Token and currency figures: **not
  observed**.

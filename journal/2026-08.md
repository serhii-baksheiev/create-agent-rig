...
- **reviewed** — two branches, five `prose-reviewer` rounds:
  ...
  | `docs/journal-session-tail` | 3 (authorised) | verdict on the PR              |

  Every round from the second on ran under the verdict contract #77 had just merged, so
  each answer was validated with `node .claude/scripts/verdict.mjs check …
prose-reviewer` before anything was read from it. Round 1 predates the contract and
  returned prose.

  🔴 **Rows three to five are this entry reporting on its own gate, and the last row
  says only where to look.** An earlier draft carried a disclaimer instead — that the
  branch faced its gate after the text was written, so nothing about it appeared here —
  and round 2 held on exactly that: four bullets above, the entry already reported
  round 1's HOLD, so the disclaimer contradicted its own document. Naming the round and
  pointing at the PR for its outcome is the form that neither contradicts itself nor
  claims a verdict it cannot have: an entry that restated the verdict would need
  editing again after the round that produced it, which is the regress this whole
  bullet is about.

  Round 2's blocker was fixed, then a slip inside that fix was fixed, and both commits
  were **unreviewed** — the branch's rounds were spent, `pr-ship` step 0 refused a
  third, and the `loop` skill's remedy for an exhausted cap is escalation to a human
  rather than a self-granted pass. So it was escalated, on the PR, with the choice
  stated. The owner authorised one round for those two commits. **The cap was not
  raised and the counter was not reset** — `.claude/queue.json` carries no
  `maxGateRounds`, so the next branch gets the same two rounds this one did. That is
  the same exit AR-65 took, and AR-100 is the filed follow-up for the fact that it
  lives in a PR comment written by the audited run.

  ⚠ Note the 3 in that message against a cap of 2, on a branch that had two rounds:
...
- **cost** — 2 reviewer subagents on #78 (prose-reviewer ×2); **1 `check-premises`
  pass**, at the second entry point, on this entry's own prose — it returned
  `UNMEASURED` with three blockers, one of which was this field claiming zero;
  0 `test-writer` runs, not applicable to a docs change; 3 completed full local suite
  runs plus 1 abandoned mid-flight when `origin/master` was merged in. **CI runs
  consumed: 3** — one on `docs/journal-ar-65` (`ab0f801`, green) and one `push` run on
  `master` for each merge (`2c13183`, `cda2f01`, both green); none on this branch yet.
  0 deploys.

  ⚠ **Most of that is this session's word.** Only two of `cost`'s figures can be
  re-checked from outside — the 3 CI runs, from the Actions API, and the 0 deploys. The

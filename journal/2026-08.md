...
- **done** — #77 (`2c13183`, AR-65) and #78 (`cda2f01`, the AR-65 journal entry).
  AR-65 transitioned to `Done`, which released AR-79 and AR-85 from the queue's
  skipped list; AR-80 stayed, blocked by AR-79 alone.
...
- 🔴 **The finding is that a journal entry failed its own gate, and every blocker was
  ...
  earlier and already recorded further down the same file. Round 2 returned SHIP with
  five advisories, all five since taken.

  **The prose layer failed the way the AR-65 run said prose fails**, one level up: the
  entry documenting "five of seven blockers were the run's own prose" shipped six of
  its own, and nothing between the writing and the merge would have looked. This is
  the case AR-99 was filed for.

  And it recursed once more, which is the part worth keeping. `check-premises` at its
  second entry point — the one AR-68 added, on the run's own prose — was run against
  **this** entry before its gate, and returned `UNMEASURED` with three blockers: a
  `cost` field claiming zero `check-premises` passes while being written by one, a
  gate round claimed on this branch that the counter had never recorded because step 0
  had not run yet, and an unobservable superlative about another session's behaviour.
  Three levels of prose about prose failing, each level failing, each caught only
  because something was pointed at it.
...
- **reviewed** — #78 spent both of the rounds its branch gets:
  ...
  | -------------------- | ----- | ------------------ |
  | `docs/journal-ar-65` | 1     | HOLD, 6 blockers   |
  | `docs/journal-ar-65` | 2     | SHIP, 5 advisories |
...
- **the advisories were deferred, not absorbed** — #78 merged with round 2's five
  advisories untaken and listed in its PR body. Both rounds were spent, so any edit
  would have shipped text no cold reader had read, which is the defect the same entry
  records #77 committing. They are taken here instead, on a branch with a fresh round.
...
- **cost** — 2 reviewer subagents on #78 (prose-reviewer ×2); **1 `check-premises`
  ...
  ⚠ Two of those figures are not re-checkable from the repository: `.claude/runs/` and
  `.claude/gate-rounds.json` are gitignored, so a later reader can verify the CI counts
  through the API but not the round counts.
...
- ⚠ **Everything above was corrected by a later session, and the corrections sit in
  ...
  `Done` for a day. Eight advisories were taken across two rounds — among them the
  ...
  five of seven blockers were the run's own prose shipped with six of its own, and
  hiding that would delete the finding. The session that made the corrections has its
  own entry above.

...
describe('rules/invariants.md — the pattern, stated once', () => {
  ...
  it('limits upstream test pointers to manifest-proven inherited artifacts that disclose the exception', async () => {
    ...
    const exception = inheritedArtifacts.split(/\nIf a hook matters/)[0] ?? '';

    expect(exception).toMatch(/exception/i);
    expect(exception).toMatch(/generator-authored|authored by (?:the )?generator/i);
...
describe('composition', () => {
  ...
  // RP-186: this pointer lives in AGENTS.md now (the canonical rulebook) —
  // CLAUDE.md is a short shim that imports it.

...
describe('the gate skill and the rules point at the router', () => {
  ...
  it('AGENTS.md no longer claims code-reviewer runs before every PR', async () => {
    // The map is the document a session reads first — AGENTS.md is that
    // document now (RP-179: Claude Code reads it natively since v2.1.277;
    // CLAUDE.md is a one-line `@AGENTS.md` shim with no prose of its own) —
    // and it was the one artifact still stating the pre-router rule: two
    // answers to one question, with the wrong one in the more prominent place.

 ...
 * differ. The first three groups read `packages/cli/src/policy/` — the
 ...
 *   bare package; no code under `src/policy/` names `eval`, `Function`,
 *   `require`, `process`, `globalThis`, `fetch` or a non-literal `import(`
 *   — comments stripped first, with a positive control).
 *
...
const sameSet = (left: readonly string[], right: readonly string[]): boolean =>
  ...
  left.every((tool) => right.includes(tool));

/**
 * Forward: every policy's native surface has a group in the snapshot whose
 * matcher names the SAME tool set — not a superset — and which wires the hook.
 * A subset check was the first shape here, and it let a snapshot that widened
 * past the declaration stay green; the mutation tests below hold both
 * directions.
 */
const missingSurfaces = (
  ...
  for (const policy of policies) {
    ...
    const covered = groups.some((group) => {
      ...
      const matches = sameSet(tools, groupTools);
...
const undeclaredHooksIn = (
  ...
  for (const group of snapshot.hooks.PreToolUse ?? []) {
    for (const hook of group.hooks) {
      const hookFile = new RegExp(`${escapeRegExp(SHARED_HOOKS_DIR)}/([a-z0-9-]+)\\.mjs`, 'g');
      for (const match of hook.command.matchAll(hookFile)) {
        const name = match[1] ?? '';
        const hookPath = `${SHARED_HOOKS_DIR}/${name}.mjs`;
...
describe('dependency direction inside src/policy', () => {
  ...
  // Whole-word tokens, so `Function(...)` without `new`, `process.getBuiltinModule`,
  // `globalThis['ev'+'al']` and `fetch(...)` are caught as well as the two obvious
  // spellings; the tokens are search strings handed to a regex, nothing is executed.
  const INTERPRETER_TOKENS = ['eval', 'Function', 'require', 'process', 'globalThis', 'fetch'];
  // Comments are prose and may say "require" or "process"; the scan is of CODE,
  // so block and line comments are stripped first. No source under src/policy
  // carries a string literal containing `//` or `/*`, which is what keeps this
  // simple stripper honest.
  const withoutComments = (text: string): string =>
    text.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/[^\n]*/g, '');
  const interpreterTokensIn = (source: string): string[] => {
    const text = withoutComments(source);
    return [
      ...INTERPRETER_TOKENS.filter((token) =>
        new RegExp(`(?<![A-Za-z0-9_$.])${token}(?![A-Za-z0-9_$])`).test(text),
      ),
      // a dynamic import whose specifier is not a string literal
      ...(/\bimport\s*\(\s*[^'"\s)]/.test(text) ? ['import(<non-literal>)'] : []),
    ];
  };

  it('the interpreter scanner reports the spellings it is asked to refuse (positive control)', () => {
    expect(interpreterTokensIn('const f = Function("return 1"); f();')).toContain('Function');
    expect(interpreterTokensIn('process.getBuiltinModule("node:fs")')).toContain('process');
    expect(interpreterTokensIn('await import(name)')).toContain('import(<non-literal>)');
    expect(interpreterTokensIn('globalThis["ev" + "al"]("1")')).toContain('globalThis');
    expect(interpreterTokensIn('await fetch("https://example.invalid")')).toContain('fetch');
    expect(interpreterTokensIn('const x = evaluate(1); import("./a.js")')).toEqual([]);
    // prose is not code: a comment may say the word
    expect(
      interpreterTokensIn('/** a policy may require it */ // process it\nconst a = 1;'),
    ).toEqual([]);
  });

  // Limit of this pin, stated: it reads one line at a time, so a multi-line
  // template literal or a regex literal carrying `//` would pass it and let the
  // stripper eat code. None exists under src/policy today; the pin is what
  // notices the single-line case, not a proof for every shape.
  it('no source under src/policy hides a comment marker inside a string, so the stripper is exact', async () => {
    for (const rel of await listTs(policyDir)) {
      const text = await readPolicyFile(rel);
      expect(/['"`][^'"`\n]*(?:\/\/|\/\*)[^'"`\n]*['"`]/.test(text), `${rel}`).toBe(false);
    }
  });

  it('nothing under src/policy evaluates text as code or reaches a global runtime surface (no interpreter)', async () => {
    for (const rel of await listTs(policyDir)) {
      const found = interpreterTokensIn(await readPolicyFile(rel));
      expect(
        found,
        `${rel} reaches an interpreter or runtime surface: ${found.join(', ')}`,
      ).toEqual([]);
    }
  });
});
...
describe('declaration and native surface correspond', () => {
  ...
  it.each(HARNESS_ADAPTERS.map((a) => [a.harness, a] as const))(
    'reports the no-verify policy on %s when the snapshot gains a tool the adapter does not name (mutation: widened snapshot)',
    (_harness, adapter) => {
      const snapshot = snapshotWith(REAL_FILE_EDIT, `${REAL_SHELL}|Task`);
      expect(missingSurfaces(adapter, POLICIES, snapshot)).toEqual([
        {
          harness: adapter.harness,
          policyId: 'no-verify-refusal',
  ...
  it.each(HARNESS_ADAPTERS.map((a) => [a.harness, a] as const))(
    'reports a policy on %s whose adapter matcher drops a tool the snapshot still wires (mutation: narrowed adapter)',
    (_harness, adapter) => {
      const narrowed: HarnessAdapter = {
        ...adapter,
        nativeSurfaceOf: (policy) => {
          const surface = adapter.nativeSurfaceOf(policy);
          return policy.policyId === 'no-verify-refusal'
            ? { ...surface, matcher: 'Bash' }
            : surface;
        },
      };
      const snapshot = snapshotWith(REAL_FILE_EDIT, REAL_SHELL);
      expect(missingSurfaces(narrowed, POLICIES, snapshot)).toEqual([
        {
          harness: adapter.harness,
          policyId: 'no-verify-refusal',
          hookPath: '.claude/hooks/block-no-verify.mjs',

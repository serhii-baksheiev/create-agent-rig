...
const loadDeclaredPaths = async (): Promise<string[]> => {
  ...
  // RP-186: AGENTS.md is the canonical rulebook and carries the block.
...
describe('dogfooding: the tool repo runs its own agent-os', () => {
  ...
  // RP-186: AGENTS.md is the composed, canonical rulebook that carries the
  // addendum; CLAUDE.md is a short shim with none of this text — it imports
  // AGENTS.md instead.
  ...
  it('declares elevated paths that actually exist in THIS repo', async () => {
    ...
    // RP-186: AGENTS.md is the canonical rulebook and carries the
    // elevated-paths block; CLAUDE.md is a short shim that imports it and
  ...
  it('declares every file that declares elevated paths of its own', async (ctx) => {
    ...
    for (const relFile of markdownFiles) {
      // The root AGENTS.md is skipped, and NOT because the assertion would be
      ...
      // (RP-186: AGENTS.md is the canonical file that carries the block now;
      // CLAUDE.md never has one to skip.)
      if (relFile === 'AGENTS.md') continue;

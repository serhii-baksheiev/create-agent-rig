...
const loadDeclaredPaths = async (): Promise<string[]> => {
  ...
  // AGENTS.md, not CLAUDE.md: AGENTS.md is the canonical rulebook (RP-179:
  // Claude Code reads it natively since v2.1.277); CLAUDE.md is a one-line
  // `@AGENTS.md` shim with no block of its own.
...
describe('dogfooding: the tool repo runs its own agent-os', () => {
  ...
  it('the composed AGENTS.md names this repo and keeps the repo addendum', async () => {
    // AGENTS.md carries the composed content now; CLAUDE.md is the one-line
    // `@AGENTS.md` shim checked separately below.
    const agentsMd = await readFile(path.join(repoRoot, 'AGENTS.md'), 'utf8');
    expect(agentsMd).toContain('create-agent-rig');
    expect(agentsMd).toContain('generator repo addendum');
    ...
    const [universalPart] = agentsMd.split('generator repo addendum');
    expect(universalPart).not.toContain('__PROJECT_NAME__');
  });

  it('CLAUDE.md is the @AGENTS.md shim, and restates none of the composed rulebook', async () => {
  ...
  it('declares every file that declares elevated paths of its own', async (ctx) => {
    ...
    for (const relFile of markdownFiles) {
      // No exemption needed for the root CLAUDE.md any more: it is now a
      // one-line `@AGENTS.md` shim with no `elevated-paths` block of its own,
      // so it never reaches `declaringFiles` in the first place. The file
      // that DOES declare one now, root AGENTS.md, needs no exemption
      // either — it is directly named in `ELEVATED_PATHS` (`'AGENTS.md'`),
      // so the coverage check below passes it honestly rather than by a
      // bespoke carve-out.

...
describe('upgrade and the opt-in workflow layer (RP-180)', () => {
  ...
  // (`.claude/skills/`, `.agents/skills/`) to the process layer. The
  // full-install figure moved once more (98→101) when RP-203 added the
  // `release-propose` skill's two shipped copies plus
  // `.claude/scripts/release-evidence.mjs`; the core-only figure stayed 62
  // because all three are workflow-layer only. The full-install figure moved
  ...
  // .claude/hooks/record-dispatch.mjs, also workflow-layer only. It moved
  // once more (103→104) when RP-228 added .claude/scripts/token-report.mjs,
  // also workflow-layer only — the core-only figure stayed 62 for the same
  // reason.
